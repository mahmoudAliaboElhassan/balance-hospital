...
export const getShiftHoursTypesForRoster = createAsyncThunk(
  "shiftHoursTypeSlice/getShiftHoursTypesForRoster",
  async (filters = {}, thunkAPI) => {
    const { rejectWithValue } = thunkAPI
    ...
    try {
      ...
      // Build the URL with query parameters
      const url = `/api/v1/RosterManagement/available-shift-types${
        queryParams.toString() ? `?${queryParams.toString()}` : ""
      }`
