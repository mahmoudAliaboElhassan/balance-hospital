...
export const exportDoctorReportToExcel = async (report, currentLang, t) => {
    ...
    report.hoursAnalysis.rosterDetails.forEach((roster) => {
        ...
        hoursSheet.getCell(currentRow, 4).value = `${roster.compliancePercentage.toFixed(1)}%`
    ...
    const chartData = [
        ...
        [isArabic ? "خروج مبكر" : "Early Checkout", report.attendanceChart.earlyCheckout],
    ...
    if (report.attendanceRecords && report.attendanceRecords.length > 0) {
        ...
        report.attendanceRecords.forEach((record) => {
            ...
            const formatTime = (dateStr) => {
                if (!dateStr) return "-"
                return new Date(dateStr).toLocaleTimeString(isArabic ? "ar-EG" : "en-US", {
                    hour: "2-digit",
                    minute: "2-digit",
                })
            }
            ...
            attendanceSheet.getCell(currentRow, 7).alignment = { horizontal: "center" }
            attendanceSheet.getCell(currentRow, 8).value = record.lateMinutes || 0
            attendanceSheet.getCell(currentRow, 8).alignment = { horizontal: "center" }
    ...
    monthlyTitle.value = isArabic ? "التوزيع الشهري للنوبات" : "Monthly Shift Distribution"
    monthlyTitle.fill = {
        type: "pattern",
        pattern: "solid",
        fgColor: { argb: "FF1E40AF" },
    ...
    if (report.rolesHistory && report.rolesHistory.length > 0) {
        ...
        report.rolesHistory.forEach((role) => {
            ...
            rolesSheet.getCell(currentRow, 1).value = isArabic ? role.roleNameAr : role.role
            rolesSheet.getCell(currentRow, 2).value = role.isActive ? (isArabic ? "نعم" : "Yes") : (isArabic ? "لا" : "No")
            rolesSheet.getCell(currentRow, 2).alignment = { horizontal: "center" }
            rolesSheet.getCell(currentRow, 3).value = role.isExpired ? (isArabic ? "نعم" : "Yes") : (isArabic ? "لا" : "No")
        ...
        link.download = `Doctor_Report_${doctorName.replace(/\s+/g, "_")}_${new Date().toISOString().split("T")[0]
            }.xlsx`
        link.click()
